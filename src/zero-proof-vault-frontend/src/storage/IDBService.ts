...
export type VaultData = {
  // BE: flexible_grid_columns : vec record { text; record { nat32; bool } };
  //     —> [ columnName: string, { nat32, bool } ] but that's ambiguous;
  flexible_grid_columns: Array<{ name: string; meta: { index: number; hidden: boolean } }>;

  // BE: secure_notes : vec record { text; text };
  //     —> [noteId, content]
  secure_notes: Array<{ id: string; content: string }>;

  // BE: flexible_grid : vec record { FlexGridDataKey; text };
  //     —> { {col,row}, value }
  flexible_grid: Array<{ key: FlexGridDataKey; value: string }>;

  // BE: website_logins : vec record { text; vec record { text; text } };
  //     —> [siteName, [[fieldKey, fieldValue], ...]]
